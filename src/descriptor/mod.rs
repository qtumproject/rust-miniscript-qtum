...
use qtum::address::WitnessVersion;
use qtum::hashes::{hash160, ripemd160, sha256};
use qtum::{secp256k1, Address, Network, Script, ScriptBuf, TxIn, Witness};
use sync::Arc;
...
impl Descriptor<DescriptorPublicKey> {
    ...
    /// Convert all the public keys in the descriptor to [`qtum::PublicKey`] by deriving them or
    /// otherwise converting them. All [`qtum::secp256k1::XOnlyPublicKey`]s are converted to by adding a
    /// default(0x02) y-coordinate.
    ///
    /// This is a shorthand for:
    ///
    /// ```
    /// # use miniscript::{Descriptor, DescriptorPublicKey, qtum::secp256k1::Secp256k1};
    ...
    pub fn derived_descriptor<C: secp256k1::Verification>(
        ...
        index: u32,
    ) -> Result<Descriptor<qtum::PublicKey>, ConversionError> {
    ...
    pub fn find_derivation_index_for_spk<C: secp256k1::Verification>(
        ...
        range: Range<u32>,
    ) -> Result<Option<(u32, Descriptor<qtum::PublicKey>)>, ConversionError> {
...
impl Descriptor<DefiniteDescriptorKey> {
    ...
    pub fn derived_descriptor<C: secp256k1::Verification>(
        ...
        impl<'a, C: secp256k1::Verification>
            Translator<DefiniteDescriptorKey, qtum::PublicKey, ConversionError>
            for Derivator<'a, C>
        {
            fn pk(
                &mut self,
            ...
            translate_hash_clone!(DefiniteDescriptorKey, qtum::PublicKey, ConversionError);
        }
...
mod tests {
    ...
    use qtum::blockdata::opcodes::all::{OP_CLTV, OP_CSV};
    use qtum::blockdata::script::Instruction;
    use qtum::blockdata::{opcodes, script};
    use qtum::hashes::hex::FromHex;
    use qtum::hashes::{hash160, sha256, Hash};
    use qtum::script::PushBytes;
    use qtum::sighash::EcdsaSighashType;
    use qtum::{self, bip32, secp256k1, PublicKey, Sequence};
    ...
    pub fn script_pubkey() {
        ...
        assert_eq!(
            bare.address(Network::Qtum).unwrap_err().to_string(),
        ...
        assert_eq!(
            pkh.address(Network::Qtum,).unwrap().to_string(),
        ...
        assert_eq!(
            wpkh.address(Network::Qtum,).unwrap().to_string(),
        ...
        assert_eq!(
            shwpkh.address(Network::Qtum,).unwrap().to_string(),
        ...
        assert_eq!(
            sh.address(Network::Qtum,).unwrap().to_string(),
        ...
        assert_eq!(
            wsh.address(Network::Qtum,).unwrap().to_string(),
        ...
        assert_eq!(
            shwsh.address(Network::Qtum,).unwrap().to_string(),
    ...
    fn satisfy() {
        ...
        let pk = qtum::PublicKey::new(secp256k1::PublicKey::from_secret_key(&secp, &sk));
        ...
        impl Satisfier<qtum::PublicKey> for SimpleSat {
            fn lookup_ecdsa_sig(
                &self,
                pk: &qtum::PublicKey,
            ) -> Option<qtum::ecdsa::Signature> {
                if *pk == self.pk {
                    Some(qtum::ecdsa::Signature {
                        sig: self.sig,
        ...
        let mut txin = qtum::TxIn {
            previous_output: qtum::OutPoint::default(),
        ...
        assert_eq!(bare.unsigned_script_sig(), qtum::ScriptBuf::new());
        ...
        assert_eq!(pkh.unsigned_script_sig(), qtum::ScriptBuf::new());
        ...
        assert_eq!(wpkh.unsigned_script_sig(), qtum::ScriptBuf::new());
        ...
        assert_eq!(sh.unsigned_script_sig(), qtum::ScriptBuf::new());
        ...
        assert_eq!(wsh.unsigned_script_sig(), qtum::ScriptBuf::new());
    ...
    #[test]
    fn after_is_cltv() {
        let descriptor = Descriptor::<qtum::PublicKey>::from_str("wsh(after(1000))").unwrap();
    ...
    #[test]
    fn older_is_csv() {
        let descriptor = Descriptor::<qtum::PublicKey>::from_str("wsh(older(1000))").unwrap();
        let script = descriptor.explicit_script().unwrap();
    ...
    #[test]
    fn tr_script_pubkey() {
        let key = Descriptor::<qtum::PublicKey>::from_str(
    ...
    #[test]
    fn roundtrip_tests() {
        let descriptor = Descriptor::<qtum::PublicKey>::from_str("multi");
        assert_eq!(
    ...
    #[test]
    fn empty_thresh() {
        let descriptor = Descriptor::<qtum::PublicKey>::from_str("thresh");
        assert_eq!(
            descriptor.unwrap_err().to_string(),
            "unexpected «no arguments given»"
        )
    ...
    fn witness_stack_for_andv_is_arranged_in_correct_order() {
        ...
        let descriptor = Descriptor::<qtum::PublicKey>::from_str(&format!(
    ...
    fn parse_descriptor_key() {
        ...
        let expected = DescriptorPublicKey::Single(SinglePub {
            key: SinglePubKey::FullKey(qtum::PublicKey::from_str(
    ...
    fn test_sortedmulti() {
        fn _test_sortedmulti(raw_desc_one: &str, raw_desc_two: &str, raw_addr_expected: &str) {
            ...
            let addr_two = desc_two
                ...
                .address(qtum::Network::Qtum)
                .unwrap();
            let addr_expected = qtum::Address::from_str(raw_addr_expected)
                .unwrap()
                .assume_checked();
...
pk(03f28773c2d975288bc7d1d205c3748651b075fbc6610e58cddeeddf8f19405aa8))";
    ...
    #[test]
    fn test_context_pks() {
        let comp_key = qtum::PublicKey::from_str(
            "02015e4cb53458bf813db8c79968e76e10d13ed6426a23fa71c2f41ba021c2a7ab",
        )
        .unwrap();
        let x_only_key = qtum::key::XOnlyPublicKey::from_str(
            "015e4cb53458bf813db8c79968e76e10d13ed6426a23fa71c2f41ba021c2a7ab",
        )
        .unwrap();
        let uncomp_key = qtum::PublicKey::from_str("04015e4cb53458bf813db8c79968e76e10d13ed6426a23fa71c2f41ba021c2a7ab0d46021e9e69ef061eb25eab41ae206187b2b05e829559df59d78319bd9267b4").unwrap();
