...
impl<Pk: MiniscriptKey> Policy<Pk> {
    ...
    ///
    /// ```
    /// use miniscript::{qtum::PublicKey, policy::concrete::Policy, Translator, hash256};
    /// use std::str::FromStr;
    /// use miniscript::translate_hash_fail;
    /// use std::collections::HashMap;
    /// use miniscript::qtum::hashes::{sha256, hash160, ripemd160};
    ...
    ///
    ///     // Fail for hash types
    ///     translate_hash_fail!(String, qtum::PublicKey, ());
    /// }
    ///
    /// let mut pk_map = HashMap::new();
    /// pk_map.insert(String::from("alice_key"), qtum::PublicKey::from_str(alice_key).unwrap());
    /// pk_map.insert(String::from("bob_key"), qtum::PublicKey::from_str(bob_key).unwrap());
